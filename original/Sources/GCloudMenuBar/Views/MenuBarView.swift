...
struct MenuBarView: View {
    @EnvironmentObject var manager: GCloudManager
    ...
    private var authStatusSection: some View {
        ...
        .padding(.horizontal, 14)
        .padding(.vertical, 10)
        .background(manager.authStatus == .expiringSoon(expiresAt: Date()) ? Color.yellow.opacity(0.05) : Color.clear)
    }
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            if showAccounts {
                ...
                HStack(spacing: 8) {
                    ...
                    Button("+ App Default") { manager.loginApplicationDefault() }
                        .buttonStyle(LinkButtonStyle())
                }
                .padding(.horizontal, 14)
                .padding(.vertical, 8)
...
struct AccountRow: View {
    ...
    var body: some View {
        ...
        .padding(.horizontal, 14)
        .padding(.vertical, 6)
        .background(account.isActive ? Color.blue.opacity(0.05) : Color.clear)
    }
}
...
// MARK: - Equatable helpers for switch pattern matching
...
extension AuthStatus {
    static func == (lhs: AuthStatus, rhs: AuthStatus) -> Bool {
        ...
        case (.authenticated(let a), .authenticated(let b)):
            return a == b
        case (.expiringSoon(let a), .expiringSoon(let b)):
            return a == b
        default:
            return false
        }
